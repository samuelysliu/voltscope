...
def source_phrases(candidate: ContentCandidate) -> list[str]:
    text = " ".join(item for item in [candidate.source_excerpt or "", candidate.raw_text_excerpt or ""] if item)
    parts = re.split(r"[.!?\n]+", text)
...
def sentence_overlap_ratio(candidate: ContentCandidate, article_text: str) -> float:
    article_sentences = [
        " ".join(part.split()) for part in re.split(r"[.!?\n]+", strip_html(article_text).lower()) if len(token_list(part)) >= 5
    ]
    max_ratio = 0.0
...
SOURCE_LINK_CTA_PHRASES = (
    ...
    "for more details see",
    "refer to the source for more",
    "read the full article at",
    "learn more at",
)


def source_link_cta_matches(value: str) -> list[str]:
...
def run_quality_gates(
    ...
    for locale, payload in (("zh-TW", zh_payload), ("en", en_payload)):
        for key in ("title", "excerpt", "html", "text", "seo_title", "seo_description"):
            if not payload.get(key):
                issues.append(
                    issue("missing_required_field", "critical", "Generated article is missing a required field", locale=locale, field=key)
                )
    ...
    if notes.get("should_publish") is False:
        issues.append(
            issue("factual_notes_should_not_publish", "critical", "Factual notes marked this candidate as unsuitable for publishing")
        )
    ...
    if overlap > settings.content_pipeline_max_source_sentence_overlap:
        issues.append(
            issue(
                ...
                "critical",
                "Generated article overlaps too closely with source text",
                value=overlap,
                threshold=settings.content_pipeline_max_source_sentence_overlap,
            )
        )
    ...
    if zh_chars < settings.content_pipeline_min_zh_chars:
        issues.append(
            issue(
                "zh_article_short",
                "critical",
                "Chinese article is shorter than the publishable minimum",
                locale="zh-TW",
                value=zh_chars,
                target=settings.content_pipeline_min_zh_chars,
            )
        )
    if en_words < settings.content_pipeline_min_en_words:
        issues.append(
            issue(
                "en_article_short",
                "critical",
                "English article is shorter than the publishable minimum",
                locale="en",
                value=en_words,
                target=settings.content_pipeline_min_en_words,
            )
        )
    ...
    if len(candidate.source_title) >= 20 and candidate.source_title.lower() in zh_title.lower():
        issues.append(
            issue("zh_title_contains_source_title", "critical", "Chinese headline must not reuse the complete English source headline")
        )
    ...
    if matched_generic_phrases:
        issues.append(
            issue(
                ...
                "critical",
                "Generated article contains generic template or editorial-process language",
                phrases=matched_generic_phrases,
            )
        )
    ...
    for image_url in image_urls or []:
        if image_url and domain(image_url) == domain(candidate.source_url):
            issues.append(
                issue("source_image_hotlink", "critical", "Generated article must not hotlink source images", image_url=image_url)
            )
