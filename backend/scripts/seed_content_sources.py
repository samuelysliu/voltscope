...
DEFAULT_SOURCES = [
    {
        ...
        "list_url": "https://autos.yahoo.com.tw/tag/electric",
        "rss_url": None,
...
async def main() -> None:
    async with AsyncSessionLocal() as session:
        for item in DEFAULT_SOURCES:
            ...
            source = await session.scalar(select(SourceWhitelist).where(SourceWhitelist.domain == domain, SourceWhitelist.name == item["name"]))
            if source is None:
